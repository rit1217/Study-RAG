# Legal QA (Flash Model)

Single-agent RAG approach that queries all 3 file stores and produces a structured legal opinion in one call using `gemini-3-flash-preview`.
...
rag = LegalRAGClient()
prompt = rag.load_prompt("legal_qa", "legal_qa", "v02")
...
1. **มาตรากฎหมายที่เกี่ยวข้อง** — List all relevant statutes as bullet points. Data from File Store only. Copy verbatim. Prioritize specific law over general law.
